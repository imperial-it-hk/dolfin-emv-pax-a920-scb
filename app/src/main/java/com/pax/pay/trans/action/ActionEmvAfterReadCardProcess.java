...
public class ActionEmvAfterReadCardProcess extends AAction {
    ...
    public ActionEmvAfterReadCardProcess(ActionStartListener listener) {
        super(listener);
    }
...
//            }
        ...
        public void run() {
            try {
                FinancialApplication.getApp().register(ActionEmvAfterReadCardProcess.this);
                CTransResult result = emvTransProcess.afterReadCardProcess(transData, emvListener);
                ...
                FinancialApplication.getApp().unregister(ActionEmvAfterReadCardProcess.this);
            }
        }
        ...
        private void handleException(EmvException e) {
            ...
            if (e.getErrCode() == EEmvExceptions.EMV_ERR_FORCE_SETTLEMENT.getErrCodeFromBasement()) {
                ...
                if (e.getErrCode() != EEmvExceptions.EMV_ERR_UNKNOWN.getErrCodeFromBasement()) {
                    if (e.getErrCode() == EEmvExceptions.EMV_ERR_UNSUPPORTED_TRANS.getErrCodeFromBasement()) {
                        transProcessListener.onShowErrMessage(e.getErrMsg(), Constants.FAILED_DIALOG_SHOW_TIME, true);
                        ...
                        if (respMsg != null && transData.getResponseCode().getCode().equals("00")) {
        ...
        private void showMsgHostReject(String respMsg) {
            if ("LE".equals(transData.getResponseCode().getCode())) {
                transProcessListener.onShowErrMessage(Component.unpackBit63Credit(transData.getField63RecByte(), transData.getField63()), Constants.FAILED_DIALOG_SHOW_TIME, true);
            } else {
    ...
    public void setResult(ActionResult result) {
        ...
        if (sTemp != null && sTemp.length > 0) {
            sTemp[0] &= 0x3F;
            if (sTemp[0] == 0x02 && sTemp[2] == 0x00) { // online PIN and CVM result will be set to 'unknown'
