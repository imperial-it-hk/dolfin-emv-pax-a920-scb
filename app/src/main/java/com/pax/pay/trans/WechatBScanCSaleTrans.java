 ...
 */
public class WechatBScanCSaleTrans extends BaseTrans {
    private String mAmount = null;
    private String refSaleID = null;
    private String qrCodeData;

    public WechatBScanCSaleTrans(Context context, TransEndListener transListener) {
        super(context, ETransType.QR_WECHAT_SCAN, Constants.ACQ_WECHAT, transListener);
        setBackToMain(true);
    }


    public WechatBScanCSaleTrans(Context context, String amount, TransEndListener transListener) {
        super(context, ETransType.QR_WECHAT_SCAN, Constants.ACQ_WECHAT, transListener);
        setBackToMain(true);
        this.mAmount = amount;
    }

    public WechatBScanCSaleTrans(Context context, String amount, String refSaleID, TransEndListener transListener) {
        super(context, ETransType.QR_WECHAT_SCAN, Constants.ACQ_WECHAT, transListener);
        setBackToMain(true);
    ...
    protected void bindStateOnAction() {
        ...
        ActionEnterAmount amountAction = new ActionEnterAmount(new AAction.ActionStartListener() {
            ...
            public void onStart(AAction action) {
                ((ActionEnterAmount) action).setParam(getCurrentContext(),
                        getString(R.string.trans_wechat_scan), false);
            }
        });
        ...
        ActionScanQRPay actionScanQRPay = new ActionScanQRPay(new AAction.ActionStartListener() {
            ...
            public void onStart(AAction action) {
                ((ActionScanQRPay) action).setParam(getCurrentContext(),
                        getString(R.string.trans_scan_wechat_sale), transData);
            }
        });
        ...
        PrintTask printTask = new PrintTask(getCurrentContext(), transData, PrintTask.genTransEndListener(WechatBScanCSaleTrans.this, State.PRINT.toString()));
        ...
        //add issue
        Issuer issuer = FinancialApplication.getAcqManager().findIssuer(Constants.ISSUER_WECHAT);
    ...
    public void onActionResult(String currentState, ActionResult result) {
        ...
        Log.i(TAG, "onActionResult: state: " + state + " ret: " + ret);

        if (state == state.ONLINE_PROCESS && result.getRet() != TransResult.SUCC) {
            transEnd(result);
            return;
        }
