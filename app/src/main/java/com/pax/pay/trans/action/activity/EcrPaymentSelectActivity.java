...
public class EcrPaymentSelectActivity extends BaseActivityWithTickForAction {
    // For log module
    ...
    protected int getLayoutId() {
        return R.layout.activity_ecr_payment_selector;
    }
    ...
    protected String getTitleString() {
        if (Arrays.equals(HyperComMsg.instance.getTransactionCode(), new byte[]{'9', '1'})) {
            ...
            if (includeQRPayment) {
                return getString(R.string.title_erc_payment_select_qr);
            } else {
                return getString(R.string.title_erc_payment_select_credit);
            }
    ...
    protected void initViews() {
        ...
        tvLabel.setText(getString(R.string.amount_widget_label));

        TextView tvUserGuide = (TextView) findViewById(R.id.txv_trans_tips);
        tvUserGuide.setText(getString(R.string.title_erc_payment_select_userguide));
    ...
    private MenuPage createMenu() {
        ...
        MenuPage.Builder builder = new MenuPage.Builder(EcrPaymentSelectActivity.this, 9, 3);
        ...
        if (includeQRPayment) {
            if (acqKPLUS != null) {
                builder.addTransItem(getString(R.string.trans_kplus), R.drawable.icon_thaiqr, getKPlusTrans());
                Log.d(TAG, "process as binding-menu-mode : ThaiQR SaleTransaction");
            }
            if (acqAlipay != null) {
                builder.addTransItem(getString(R.string.trans_alipay), R.drawable.icon_alipay, getAlipayTrans());
                Log.d(TAG, "process as binding-menu-mode : Alipay SaleTransaction");
            }
            if (acqWechat != null) {
                builder.addTransItem(getString(R.string.trans_wechat), R.drawable.icon_wechatpay, getWechatTrans());
                Log.d(TAG, "process as binding-menu-mode : ThaiQR SaleTransaction");
            }
            if (acqQrCredit != null) {
                builder.addTransItem(getString(R.string.trans_qr_credit), R.drawable.app_sale, getQrCreditTrans());
    ...
    protected void setListeners() {
        transEndListener = new ATransaction.TransEndListener() {
            ...
            public void onEnd(ActionResult result) {
                ...
                if (!isReturnedToEcrLinkPos) {finish(new ActionResult(result.getRet(), null));}
                finish();
                ...
                for (Acquirer acquirer : acquirerList) {
                    try {
                        ...
                        if (localHostNII.equals("999")) {
                            ret = Printer.printAuditReport(EcrPaymentSelectActivity.this, acquirer);
                        } else {
                ...
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (printcount > 0) {
                            ...
                            DialogUtils.showErrMessage(EcrPaymentSelectActivity.this, Utils.getString(R.string.linkpos_ecr_audit_report_printing), "Error : No Record ", onAuditReportPrintDismissListener, Constants.FAILED_DIALOG_SHOW_TIME);
                        }
                    }
                });
            }
    ...
    private void autoOpenTrans() {
        if (search_RefSaleID_transData(refSaleID)) {
            ...
            if (lastTransDataRecord.getTransState() == TransData.ETransStatus.VOIDED) {
                ...
                if (FinancialApplication.getEcrProcess().mHyperComm instanceof LawsonHyperCommClass && Arrays.equals(HyperComMsg.instance.getTransactionCode(), "70".getBytes())) {
                    HyperComMsg.instance.setTransactionCode(new byte[] {'7','1'});
                }
                returnSaleResponse();
    ...
    private SaleTrans getCreditDebitCardSaleTrans() {
        return getCreditDebitCardSaleTrans(false);
    }
    ...
    private DolfinSaleTran getDolfinTrans() {
        return getDolfinTrans(false);
    }
    ...
    protected void onTimerTick(long timeleft) {
        btn_cancel.setText(String.format(getString(R.string.title_erc_payment_select_cancel_button_with_timer).toString(), timeleft));
    ...
    private void returnRejectResponse(byte[] respCode) {
        if (mHyperComm != null) {
            try {
                if (HyperComMsg.instance.getTransactionCode() != null && EcrData.instance.isOnProcessing) {
