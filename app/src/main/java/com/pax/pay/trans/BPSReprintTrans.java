...
public class BPSReprintTrans extends BaseTrans {

    private TransData transData;
    private TransMultiAppData multiAppLastTrans;
    private long origTraceNo = -1;
    private long lastTraceNo = -1;
    private List<Acquirer> supportAcquirers;
    private String swapMerchantName = null;

    public BPSReprintTrans(Context context, TransEndListener transListener) {
    ...
    protected void bindStateOnAction() {
        ...
        ActionInputTransData enterTransNoAction = new ActionInputTransData(new AAction.ActionStartListener() {
            ...
            public void onStart(AAction action) {
                ...
                ((ActionInputTransData) action).setParam(getCurrentContext(), getString(R.string.menu_reprint))
                        .setInputLine(promptMsg, ActionInputTransData.EInputType.NUM, 6, true);
            }
        });
        bind(BPSReprintTrans.State.ENTER_TRANSNO.toString(), enterTransNoAction, true);
        ...
        ActionDispTransDetail confirmInfoAction = new ActionDispTransDetail(new AAction.ActionStartListener() {
            ...
            public void onStart(AAction action) {
                ...
                ((ActionDispTransDetail) action).setParam(getCurrentContext(),
                        getString(R.string.menu_reprint), getShowDetail(map));
            }
        });
        bind(BPSReprintTrans.State.TRANS_DETAIL.toString(), confirmInfoAction, true);
        ...
        bind(BPSReprintTrans.State.PRINT.toString(), printReceiptAction, true);
        ...
        bind(BPSReprintTrans.State.PRINT_DOLFIN.toString(), actionDolfinPrintTran, true);
        ...
        bind(BPSReprintTrans.State.PRINT_SCB_IPP_REDEEM.toString(), actionScbIppReprint, true);

        ActionAmexReprint actionAmexReprint = new ActionAmexReprint(action ->
                ((ActionAmexReprint) action).setParam(getCurrentContext(), origTraceNo, lastTraceNo, -1)
        );
        bind(BPSReprintTrans.State.PRINT_AMEX_API.toString(), actionAmexReprint);

        this.getSupportAcquirers();// init default acquirers.

        gotoState(BPSReprintTrans.State.ENTER_TRANSNO.toString());
    }
    ...
    public void onActionResult(String currentState, ActionResult result) {
        BPSReprintTrans.State state = BPSReprintTrans.State.valueOf(currentState);
        ...
        switch (state) {
            ...
            case TRANS_DETAIL:
                switch (transData.getAcquirer().getName()) {
                    case Constants.ACQ_DOLFIN:
                        gotoState(State.PRINT_DOLFIN.toString());
                        break;
                    case Constants.ACQ_SCB_IPP:
                    case Constants.ACQ_SCB_REDEEM:
                        gotoState(State.PRINT_SCB_IPP_REDEEM.toString());
                        break;
                    case Constants.ACQ_AMEX:
                        gotoState(State.PRINT_AMEX_API.toString());
                        break;
                    default:
            ...
            case PRINT_AMEX_API:
                if (result.getRet() == TransResult.SUCC) {
                    ...
                    } else if (result.getRet() == TransResult.ERR_ABORTED && transData == null) {
                        transEnd(new ActionResult(TransResult.ERR_NO_ORIG_TRANS, null));
                    } else {
    ...
    private void onEnterTraceNo(ActionResult result) {
        ...
        if (content == null) {
            ...
            if (multiAppLastTrans != null && traceNo < multiAppLastTrans.getTraceNo()) {
                goToPrintTransByMultiApp(multiAppLastTrans, multiAppLastTrans.getTraceNo());
                return;
            }
...
//            }
    ...
    private void validateOrigTransData(long origTransNo) {
        ...
        if (transData == null) {
            // trans not exist
            TransMultiAppData transMultiAppData = FinancialApplication.getTransMultiAppDataDbHelper().findTransDataByTraceNo(origTransNo);

            if (transMultiAppData != null) {
                goToPrintTransByMultiApp(transMultiAppData, origTransNo);
            } else {
...
//            return;
        ...
        if (MerchantProfileManager.INSTANCE.isMultiMerchantEnable()) {
            if (!transData.getMerchantName().equals(MerchantProfileManager.INSTANCE.getCurrentMerchant())) {
                swapMerchantName = MerchantProfileManager.INSTANCE.getCurrentMerchant();
                MerchantProfileManager.INSTANCE.applyProfileAndSave(transData.getMerchantName());

                String txnMercName = transData.getMerchantName();
                String txnAcqName = transData.getAcquirer().getName();
                if(txnMercName!=null && txnAcqName!=null) {
                    ...
                    if (mercAcqProfile!=null) {
                        transData.getAcquirer().setTerminalId(mercAcqProfile.getTerminalId());
                        transData.getAcquirer().setMerchantId(mercAcqProfile.getMerchantId());
                        transData.getAcquirer().setCurrBatchNo(mercAcqProfile.getCurrBatchNo());
                    }
                }
            }
        }
        ...
        if (transData.getTransState() == TransData.ETransStatus.VOIDED) {
        ...
        } else if (transData.getTransState() == TransData.ETransStatus.SALE_COMPLETED) {
            ...
            TransData trans = FinancialApplication.getTransDataDbHelper().findTransDataByTraceNo(transData.getTraceNo(), true, excludeTrans);
            if (trans != null) {
                transData = trans;
            }
        }
        ...
        gotoState(BPSReprintTrans.State.TRANS_DETAIL.toString());
    }

    private void goToPrintTransByMultiApp(TransMultiAppData transMultiAppData, long origTransNo) {
    ...
    private LinkedHashMap<String, String> getShowDetail(LinkedHashMap<String, String> map) {
        ...
        switch (acqName) {
            ...
            case Constants.ACQ_DOLFIN_INSTALMENT:
                ...
                break;
            case Constants.ACQ_SCB_IPP:
                transType = "SCB IPP" + (transState == TransData.ETransStatus.VOIDED ? " (" +getString(R.string.trans_void)+ ")" : "");
                map.put(getString(R.string.history_detail_type), transType);
                map.put(getString(R.string.history_detail_amount), CurrencyConverter.convert(amount, transData.getCurrency()));
                map.put(getString(R.string.history_detail_card_no), PanUtils.maskCardNo(transData.getPan(), transData.getIssuer().getPanMaskPattern()));
                map.put(getString(R.string.history_detail_auth_code), transData.getAuthCode());
                ...
                break;
            case Constants.ACQ_SCB_REDEEM:
                long redeemAmt = Utils.parseLongSafe(transData.getRedeemedAmount(), 0);
                redeemAmt = transState == TransData.ETransStatus.VOIDED ? -redeemAmt : redeemAmt;
                transType = "SCB REDEMPTION" + (transState == TransData.ETransStatus.VOIDED ? " (" +getString(R.string.trans_void)+ ")" : "");
                map.put(getString(R.string.history_detail_type), transType);
                map.put(getString(R.string.history_detail_amount), CurrencyConverter.convert(amount, transData.getCurrency()));
                map.put(getString(R.string.history_detail_redeem_amount), CurrencyConverter.convert(redeemAmt, transData.getCurrency()));
                map.put(getString(R.string.history_detail_card_no), PanUtils.maskCardNo(transData.getPan(), transData.getIssuer().getPanMaskPattern()));
                map.put(getString(R.string.history_detail_auth_code), transData.getAuthCode());
                map.put(getString(R.string.history_detail_ref_no), transData.getRefNo());
                break;
            default:
                ...
                if (transData.isDccRequired()) {
                    ...
                    String dccAmount;
                    if (Utils.parseLongSafe(transData.getAdjustedDccAmount(), 0) > 0) {
                        dccAmount = CurrencyConverter.convert(Utils.parseLongSafe(transData.getAdjustedDccAmount(), 0), currencyNumeric);
                    } else {
                        dccAmount = CurrencyConverter.convert(Utils.parseLongSafe(transData.getDccAmount(), 0), currencyNumeric);
                    }
    ...
    private void showRespMsg(ActionResult result){
        ...
        DialogUtils.showErrMessage(getCurrentContext(), getString(R.string.trans_reprint), respMsg, onDismissListener, Constants.FAILED_DIALOG_SHOW_TIME);
    }
