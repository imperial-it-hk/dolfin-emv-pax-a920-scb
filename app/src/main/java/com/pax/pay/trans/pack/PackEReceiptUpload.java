...
public class PackEReceiptUpload extends PackIsoBase {
    private TransData origTransData;

    public PackEReceiptUpload(PackListener listener) {
        super(listener);
...
//            if (transData.getAcquirer() == null || transData.getAcquirer().getTWK() == null) {
//                return "".getBytes();
//            }
            ...
            if (origTransData!=null) {
                transData.setInitAcquirerIndex(EReceiptUtils.StringPadding(String.valueOf(origTransData.getAcquirer().getId()),3,"0", Convert.EPaddingPosition.PADDING_LEFT));
                transData.setInitAcquirerNii(EReceiptUtils.StringPadding(String.valueOf(origTransData.getAcquirer().getNii()),3,"0", Convert.EPaddingPosition.PADDING_LEFT));
                transData.setInitAcquirerName(origTransData.getAcquirer().getName());
            }
    ...
    protected void setMandatoryData(@NonNull TransData transData) throws Iso8583Exception {
        ...
        if(origTransData !=null && (origTransData.getTransType()==ETransType.QR_INQUIRY
                    ...
                    || origTransData.getTransType()==ETransType.QR_INQUIRY_CREDIT
                    || origTransData.getTransType()==ETransType.QR_VOID_CREDIT
                    || origTransData.getTransType()==ETransType.QR_VERIFY_PAY_SLIP)
                || transData.getOrigTransType()==ETransType.KCHECKID_DUMMY) {
...
//        transData.setNii(origTransData.getAcquirer().getNii());//todo
        ...
        if (transData.getOrigTransType() == ETransType.KCHECKID_DUMMY) {
            swapAcquirer(transData);
        } else {
            setBitData41(origTransData);
            setBitData42(origTransData);
        }
...
//        setBitData54(transData);
        ...
        if (transData.getOrigTransType()==ETransType.KCHECKID_DUMMY) {
            ...
            setBitData62(origTransData);
        }

        setBitData63Byte(transData);//todo Receipt image table ???
    }

    private void swapAcquirer(TransData transData) throws Iso8583Exception {
        Acquirer targetAcquirer = null;
        Acquirer currentAcquirer = transData.getAcquirer();

        switch (transData.getOrigTransType()) {
            case KCHECKID_DUMMY:
                targetAcquirer = FinancialApplication.getAcqManager().findActiveAcquirer(Constants.ACQ_KCHECKID);
                break;
        }
        // switch to target acquirer
    ...
    protected void setBitData37(@NonNull TransData transData) throws Iso8583Exception {
        ...
        String RefNo = "";
        if(origTransData.getRefNo()!=null) {
            if(origTransData.getRefNo().equals("")) {
                RefNo = Component.getPaddedNumber(origTransData.getAcquirer().getCurrBatchNo(), 6) + Component.getPaddedNumber(origTransData.getStanNo(), 6);
            } else {
                RefNo = origTransData.getRefNo();
            }
        } else {
            RefNo = Component.getPaddedNumber(origTransData.getAcquirer().getCurrBatchNo(), 6) + Component.getPaddedNumber(origTransData.getStanNo(), 6);
        }
    ...
    protected void setBitData38(@NonNull TransData transData) throws Iso8583Exception {
        if (transData.getOrigTransType()==ETransType.KCHECKID_DUMMY) {
            setBitData("38", transData.getRefNo());
        } else {
            setBitData("38", origTransData.getAuthCode());
        }
    }
    ...
    protected void setBitData58(@NonNull TransData transData) throws Iso8583Exception {
        ...
        String[] Reformat = actionEReceiptInfoUpload.ReformatCardScheme(origTransData);
        if (Reformat!=null) {
            Log.d(EReceiptUtils.TAG,"         Payment Type = " + Reformat[0]);
            setBitData("58", Reformat[0]);                                      //todo Payment type  -- SALE,VOID, REDEEM, INSTALLMENT
        } else {
            if (transData.getOrigTransType()==ETransType.KCHECKID_DUMMY) {
                setBitData("58", "KCHECKID" );
            } else {
                setBitData("58", origTransData.getOrigTransType().getTransName() );
            }
        }
    }
    ...
    protected void setBitData59(@NonNull TransData transData) throws Iso8583Exception {
        ...
        if (Reformat!=null) {
            ...
            if (transData.getOrigTransType()==ETransType.KCHECKID_DUMMY) {
                setBitData("59", "ID-CARD" );
            } else {
                setBitData("59", origTransData.getIssuer().getName() );
            }
        }
    }
    ...
    protected void setBitData60(@NonNull TransData transData) throws Iso8583Exception {
        ...
        setBitData("60", Component.getPaddedNumber(origTransData.getAcquirer().getCurrBatchNo(), 6));
    }
    ...
    protected void setBitData61Byte(@NonNull TransData transData) throws Iso8583Exception {
        ...
        if (transData.geteSlipFormat()!=null) {
            ...
            Log.d(TAG, "missing eReceiptData to be put into field61");
        }
    }
    ...
    private byte[] getEReceiptType(TransData transData) {
        ...
        if (transData.getTransType() != ETransType.ERCEIPT_SETTLE_UPLOAD && origTransData !=null) {
            if (origTransData.getSignData() != null) {
                return "00".getBytes();
            } else if (origTransData.isPinVerifyMsg() || origTransData.isTxnSmallAmt() || !origTransData.getAcquirer().isSignatureRequired()) {
                return "02".getBytes();
            }
        }
