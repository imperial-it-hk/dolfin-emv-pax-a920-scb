...
import com.pax.pay.trans.action.ActionGetQrWechatFromServer;
import com.pax.pay.trans.action.ActionInquiryWechat;
 ...
 */
public class WechatQrSaleTrans extends BaseTrans {
    private String mAmount = null;
    private ActionResult lastResult;
    private String refSaleID = null;
    private boolean isPosManualInquiry = false;

    public WechatQrSaleTrans(Context context, TransEndListener transListener) {
        super(context, ETransType.GET_QR_WECHAT, Constants.ACQ_WECHAT, transListener);
        setBackToMain(true);
    }

    public WechatQrSaleTrans(Context context, String amount, TransEndListener transListener) {
        super(context, ETransType.GET_QR_WECHAT, Constants.ACQ_WECHAT, transListener);
        setBackToMain(true);
        this.mAmount = amount;
    }
    ...
    public WechatQrSaleTrans(Context context, String amount, String refSaleID, boolean isPosManualInquiry, TransEndListener transListener) {
        super(context, ETransType.GET_QR_WECHAT, Constants.ACQ_WECHAT, transListener);
        setBackToMain(true);
    ...
    protected void bindStateOnAction() {
        ...
        ActionEnterAmount amountAction = new ActionEnterAmount(new AAction.ActionStartListener() {
            ...
            public void onStart(AAction action) {
                ((ActionEnterAmount) action).setParam(getCurrentContext(),
                        getString(R.string.trans_wechat_sale), false);
            }
        });
        bind(WechatQrSaleTrans.State.ENTER_AMOUNT.toString(), amountAction, true);
        ...
        ActionGetQrWechatFromServer getQrFromServer = new ActionGetQrWechatFromServer(new AAction.ActionStartListener() {
            @Override
            public void onStart(AAction action) {
                ((ActionGetQrWechatFromServer) action).setParam(getCurrentContext(),
                        /*getString(R.string.menu_wallet_sale),*/ transData);
            }
        });
        bind(WechatQrSaleTrans.State.GEN_QR.toString(), getQrFromServer);
...
//                        getString(R.string.trans_wechat_sale), transData);
//            }
//        });
//        bind(State.REVERSAL_QR.toString(), GetQRReversal);
        ...
        ActionInquiryWechat qrSaleInquiry = new ActionInquiryWechat(new AAction.ActionStartListener() {
            @Override
            public void onStart(AAction action) {
                ((ActionInquiryWechat) action).setParam(getCurrentContext(),
                        getString(R.string.trans_wechat_sale), transData);
            }
        });
        ...
        ActionShowQRCodeWallet showQRCodeWallet = new ActionShowQRCodeWallet(new AAction.ActionStartListener() {
            ...
            public void onStart(AAction action) {
                ((ActionShowQRCodeWallet) action).setParam(getCurrentContext(),
                        getString(R.string.trans_wechat_sale), R.string.trans_dynamic_qr, isPosManualInquiry, transData);
            }
        });
...
//        ActionCheckQRWechat checkQR = new ActionCheckQRWechat(new AAction.ActionStartListener() {
//            @Override
//            public void onStart(AAction action) {
//                ((ActionCheckQRWechat) action).setParam(getCurrentContext(),
//                        getString(R.string.trans_wechat_sale), transData);
//            }
//        });
//        bind(WechatQrSaleTrans.State.CHECKQR.toString(), checkQR, false);

        //print preview action
        PrintTask printTask = new PrintTask(getCurrentContext(), transData, PrintTask.genTransEndListener(WechatQrSaleTrans.this, WechatQrSaleTrans.State.PRINT.toString()));
        bind(WechatQrSaleTrans.State.PRINT.toString(), printTask);
        ...
        int issuerSupportResult = IssuerSupportTransactionCheck(Collections.singletonList(Constants.ISSUER_WECHAT));
        if (issuerSupportResult != TransResult.SUCC) {
            transEnd(new ActionResult(issuerSupportResult,null));
            return;
        }

        Acquirer acq = AcqManager.getInstance().findActiveAcquirer(Constants.ACQ_WECHAT);
        ...
        if (mAmount == null) {
            ...
            if (FinancialApplication.getEcrProcess().mHyperComm instanceof LawsonHyperCommClass) {
                ...
                EcrData.instance.qr_IssuerName = Constants.ISSUER_WECHAT.getBytes();
            }
    ...
    public void onActionResult(String currentState, ActionResult result) {
        ...
        WechatQrSaleTrans.State state = WechatQrSaleTrans.State.valueOf(currentState);
        ...
        switch (state) {
            ...
            case CHECKQR:
                ...
                if (result.getRet() == TransResult.SUCC
                    ...
                    gotoState(WechatQrSaleTrans.State.INQUIRY_QR.toString());
                } else {
                    //ECRProcReturn(null, new ActionResult(result.getRet(), null));
                    //gotoState(WechatQrSaleTrans.State.PRINT.toString());
                    transEnd(result);
                }
                break;
            case PRINT:
    ...
    public void gotoState(String state) {
        if (transData.isEcrProcess() && FinancialApplication.getEcrProcess().mHyperComm instanceof LawsonHyperCommClass) {
            if (state.equals(WechatQrSaleTrans.State.PRINT.toString())) {
                if (transData != null) {
                    // set SaleReferenceID
                ...
                if (transData != null) {
                    ...
                    EcrData.instance.walletType = EcrPaymentSelectActivity.SEL_PAYMENT_QR_WCP;
                    //EcrData.instance.RefNo = Utils.checkStringContainNullOrEmpty(transData.getRefNo()).getBytes();
...
//                gotoState(WechatQrSaleTrans.State.CHECKQR.toString());
//            } else {
