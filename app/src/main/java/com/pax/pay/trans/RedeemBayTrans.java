...
public class RedeemBayTrans extends BaseTrans {
    ...
    private int type; //RedeemType 1=Full,2=Partial,3=Catalogue
     ...
     */
    public RedeemBayTrans(Context context, byte mode, boolean isFreePin,
                          TransEndListener transListener, int type) {
        super(context, ETransType.BAY_REDEEM_FREEDOM, transListener);
        this.context = context;
        setParam( mode, isFreePin, type);
    }
    ...
    private void setParam( byte mode, boolean isFreePin, int type) {
...
//        }
    ...
    public void bindStateOnAction() {
        ...
        ActionEnterRedeemBay actionEnterRedeem = new ActionEnterRedeemBay(new AAction.ActionStartListener() {
            @Override
            public void onStart(AAction action) {
                ((ActionEnterRedeemBay) action).setParam(getCurrentContext(),getString(R.string.menu_redeem_bay),transData,type);
            }
        });
        bind(State.ENTER_REDEEM.toString(), actionEnterRedeem, true);
        ...
        ActionSearchCard searchCardAction = new ActionSearchCard(new AAction.ActionStartListener() {
            ...
            @Override
            public void onStart(AAction action) {
                ((ActionSearchCard) action).setParam(getCurrentContext(), getString(R.string.menu_redeem_bay), searchCardMode, transData.getAmount(),
                        null, "", transData);
            }
        });
        ...
        PrintTask printTask = new PrintTask(getCurrentContext(), transData, PrintTask.genTransEndListener(RedeemBayTrans.this, State.PRINT.toString()));
        bind(State.PRINT.toString(), printTask);

        gotoState(State.ENTER_REDEEM.toString());
...
//            }
    ...
    private void onRedeemAmt(ActionResult result){
        ActionEnterRedeemBay.RedeemBayInfo redeemInfo = (ActionEnterRedeemBay.RedeemBayInfo) result.getData();
        transData.setAmount(redeemInfo.getRedeemAmount().replace(".",""));
        transData.setMktCode(redeemInfo.getRedeemMktCode());
        transData.setRedeemPoints(redeemInfo.getRedeemPoints());
        transData.setSerialNum(redeemInfo.getRedeemPrdCode());


        if( !("").equals(redeemInfo.getRedeemType())){
            //Redeem Catalogue
            final String[] catalogType =  context.getResources().getStringArray(R.array.edc_redeem_bay_catalog_value_entries);
            transData.setRedeemPartial(redeemInfo.getRedeemType().equals(catalogType[0]) ? false : true);
            transData.setTransType(ETransType.BAY_REDEEM_CATALOGUE);
        }else{
            transData.setTransType(ETransType.BAY_REDEEM_FREEDOM);
        }
    }
