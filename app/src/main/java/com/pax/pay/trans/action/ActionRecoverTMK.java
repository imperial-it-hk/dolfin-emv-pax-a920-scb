...
public class ActionRecoverTMK extends AAction {
    private Context context;
    private String field62;
    private static Acquirer acq;
    private static EPedType type;

    public ActionRecoverTMK(ActionStartListener listener) {
        super(listener);
    }
    ...
    protected void process() {
        int ret = recoverTMK(field62);
        if(ret != TransResult.SUCC) {result_TMK_ID = null;}
        setResult(new ActionResult(ret, result_TMK_ID));
    }

    private static byte[] result_TMK_ID ;
    public static int recoverTMK (String ex_DE62) {
