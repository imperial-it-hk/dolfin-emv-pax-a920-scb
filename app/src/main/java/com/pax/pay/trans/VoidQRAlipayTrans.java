...
public class VoidQRAlipayTrans extends BaseTrans {
    ...
    public VoidQRAlipayTrans(Context context, TransEndListener transListener) {
        super(context, ETransType.QR_VOID_ALIPAY, transListener);
        isNeedFindOrigTrans = true;
        isNeedInputTransNo = true;
        setBackToMain(true);
    }

    public VoidQRAlipayTrans(Context context, TransData origTransData, TransEndListener transListener) {
        super(context, ETransType.QR_VOID_ALIPAY, transListener);
    ...
    protected void bindStateOnAction() {
        ...
        bind(VoidQRAlipayTrans.State.ENTER_TRACENO.toString(), enterTransNoAction, true);
        ...
        bind(VoidQRAlipayTrans.State.TRANS_DETAIL.toString(), confirmInfoAction, true);
        ...
        bind(VoidQRAlipayTrans.State.INQUIRY.toString(), qrSaleInquiry, false);

        //print preview action
        PrintTask printTask = new PrintTask(getCurrentContext(), transData, PrintTask.genTransEndListener(VoidQRAlipayTrans.this, VoidQRAlipayTrans.State.PRINT.toString()));
        bind(VoidQRAlipayTrans.State.PRINT.toString(), printTask);
        ...
        if (FinancialApplication.getSysParam().get(SysParam.BooleanParam.OTHTC_VERIFY)) {
            gotoState(VoidQRAlipayTrans.State.ENTER_TRACENO.toString());
        }
    }
    ...
    public void onActionResult(String currentState, ActionResult result) {
        VoidQRAlipayTrans.State state = VoidQRAlipayTrans.State.valueOf(currentState);
        switch (state) {
            case ENTER_TRACENO:
                onEnterTraceNo(result);
                break;
            case TRANS_DETAIL:
                gotoState(VoidQRAlipayTrans.State.INQUIRY.toString());
                break;
            case INQUIRY:
//                transData = (TransData) result.getData();
            ...
            case PRINT:
                if (result.getRet() == TransResult.SUCC) {
                    ...
                    gotoState(VoidQRAlipayTrans.State.PRINT.toString());
                }
                break;
            default:
                transEnd(result);
    ...
    private void onEnterTraceNo(ActionResult result) {
        ...
        long transNo;
        if (content == null) {
            TransData transData = FinancialApplication.getTransDataDbHelper().findLastAlipayTransData();
            if (transData == null || transData.getTransType() == ETransType.QR_VOID_ALIPAY) {
    ...
    private void validateOrigTransData(long origTransNo) {
        if(FinancialApplication.getSysParam().get(SysParam.BooleanParam.EDC_ENABLE_VOID_WITH_STAND)) {
            origTransData = FinancialApplication.getTransDataDbHelper().findAlipayTransDataByStanNo(origTransNo);
        } else{
            origTransData = FinancialApplication.getTransDataDbHelper().findAlipayTransDataByTraceNo(origTransNo);
        }
