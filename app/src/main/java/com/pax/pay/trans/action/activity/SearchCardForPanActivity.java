...
public class SearchCardForPanActivity extends BaseActivityWithTickForAction {
    ...
    private SearchCardForPanActivity.SearchCardThread searchCardThread = null;
    private SearchCardForPanActivity.SearchSP200Thread mSearchSP200Thread = null;
    private boolean isTimeOut = false;
    private ConditionVariable cv;
    private SP200_serialAPI sp200API = null;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (supportSP200) {
            sp200API = SP200_serialAPI.getInstance();
            ...
            sp200API.GetContactlessPan(amountSP200);
    ...
    protected void loadParam() {
        ...
        // 获取寻卡提醒
        if ((mode & SearchMode.WAVE) != SearchMode.WAVE && (mode & SearchMode.KEYIN) == SearchMode.KEYIN) {
        ...
        } else if ((mode & SearchMode.WAVE) != SearchMode.WAVE && (mode & SearchMode.KEYIN) != SearchMode.KEYIN) {
...
//        runInputMerchantPwdAction();
    ...
    private void onVerifyManualIssuer() {
        ...
        if (matchedIssuerList == null || matchedIssuerList.isEmpty()) {
            ...
            if (AmexTransAPI.getInstance().getProcess().isAppInstalled(this)) {
    ...
    private void onOkClicked() {
        ...
        if (pollingResult != null && pollingResult.getReaderType() == EReaderType.ICC) {
            ...
            if (enableTip && iccAdjustPercent > 0) {
                ...
                Intent intent = new Intent(SearchCardForPanActivity.this, AdjustTipActivity.class);
    ...
    private void runInputMerchantPwdAction() {
        inputPasswordAction = new ActionInputPassword(new AAction.ActionStartListener() {
            ...
            public void onStart(AAction action) {
                ((ActionInputPassword) action).setParam(SearchCardForPanActivity.this, 6,
        ...
        inputPasswordAction.setEndListener(new AAction.ActionEndListener() {
            ...
            public void onEnd(AAction action, ActionResult result) {
                ...
                if (!data.equals(FinancialApplication.getSysParam().get(SysParam.StringParam.SEC_MERCHANT_PWD))) {
                    ...
                    if (retryTime > 0) {
                        // AET-110, AET-157
                        DialogUtils.showErrMessage(SearchCardForPanActivity.this, getString(R.string.trans_password),
    ...
    private class SearchSP200Thread extends Thread {
        ...
        @Override
        public void run() {
            sp200API.StartReceiveThread(10);
            FinancialApplication.getApp().runOnUiThreadDelay(new Runnable() {
                @Override
                public void run() {
        ...
        private void onReadSP200Result(int result) {
            ...
            if (iRet == TransResult.SUCC) {
                emv = sp200API.GetPanResult(szMsgResult);
                if (emv != null) {
                    FinancialApplication.getDal().getCardReaderHelper().stopPolling();
    ...
    public class SelectIssuerRunnable implements Runnable {
        ...
        @Override
        public void run() {
            Context context = SearchCardForPanActivity.this;
