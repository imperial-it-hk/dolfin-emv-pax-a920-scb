...
open class PosnetMessage internal constructor(val model: LinkPOSModel) : ProtocolMessage {
    companion object {
        const val TAG = "PosnetMessage"
        const val TRANS_CODE_SALE_ALL_TYPE = "SALE"
        const val TRANS_CODE_SALE_VAT_B = "VATB"
        const val TRANS_CODE_VOID_TYPE = "VOID"
        const val TRANS_CODE_SETTLEMENT_TYPE = "SETT"
    ...
    override fun onProcess(mContext: Context) {
        try {
            ...
            when (model.request.transCode) {
                TRANS_CODE_SALE_ALL_TYPE -> doSale(mContext)
                TRANS_CODE_SALE_VAT_B -> doSaleVatB(mContext)
    ...
    private fun successTransResponse() {
    ...
    private fun successSaleQrPaymentTransResponse() {
        val response = LinkPOSResponse()
        response.status = "SUCCESS"
        setSuccessResponseCode(response)
        setField02(response, "APPROVE")
...
//        setFieldD2QrPayment(response)
    ...
    private fun rejectResponse(ret: Int) {
        val response = LinkPOSResponse()
    ...
    private fun rejectResponse(respCode: String) {
    ...
    private fun setFieldR1(response: LinkPOSResponse): LinkPOSResponse {
        response.r1 = String(EcrData.instance.StanNo)
        return response
    }
}